...
def upload_embedding(connection, notes_table, content, note, start_line, end_line, embedding):
    cursor = connection.cursor() 
    if(not cursor):
        print("NO CURSOR")
        return(None)
    
    query = 'INSERT INTO {} (content, note, start_line, end_line, embedding) VALUES (%s, %s, %s, %s, %s)'.format(notes_table)
    cursor.execute(query, (content, note, start_line, end_line, embedding,))

    connection.commit()

    return(connection)

def process_note(df, script, connection, notes_table='notes', note_title="", startingline=0, tokenlim=500, halt=False, embedding_model=None, *args, **kwargs):
    if(not connection):
    ...
    for i, j in script.getAllTokenChunkBounds(df, tokenlim):
        ...
        embed_response = create_embedding("\n".join(str_rows), embedding_model)
        embed = embed_response['data'][0]['embedding']

        print("{} | {} -> {}".format(note_title, i, j))
        upload_embedding(connection, notes_table, "\n".join(str_rows), note_title, i, j, np.asarray(embed))
